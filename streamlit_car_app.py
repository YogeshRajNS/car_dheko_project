...
model = joblib.load(r'C:\Users\NAGARAJAN K\Desktop\car dhekomethod3\car_price_prediction_gb_model.pkl')
label_encoders = joblib.load(r'C:\Users\NAGARAJAN K\Desktop\car dhekomethod3\label_encoders.pkl')
scalers = joblib.load(r'C:\Users\NAGARAJAN K\Desktop\car dhekomethod3\scalers.pkl')

# Load dataset for filtering and identifying similar data
data = pd.read_csv(r"C:\Users\NAGARAJAN K\Desktop\car dhekomethod3\car_dekho_Structuredraw.csv")
...
features = [
    ...
    'oem', 'model', 'modelYear', 'variantName', 
    'City', 'mileage', 'Seats', 'car_age', 
    'brand_popularity', 'mileage_normalized'
]
...
def preprocess_input(df):
    ...
    for column in ['ft', 'bt', 'transmission', 'oem', 'model', 'variantName', 'City']:
        if column in df.columns and column in label_encoders:
            df[column] = df[column].apply(lambda x: label_encoders[column].transform([x])[0] if pd.notnull(x) else -1)
...
def visual_selectbox(label, options, index=0):
    selected_option = st.selectbox(label, options, index=index)
...
# User inputs for numerical values
modelYear = st.number_input('8. Year of Manufacture', min_value=1980, max_value=2024, value=2015)
ownerNo = st.number_input('9. Number of Previous Owners', min_value=0, max_value=10, value=1)
km = st.number_input('10. Kilometers Driven', min_value=0, max_value=500000, value=10000)
...
mileage = st.slider('11. Mileage (kmpl)', min_value=min_mileage, max_value=max_mileage, value=min_mileage, step=0.5)

# Select city from the dataset
